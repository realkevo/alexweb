...
if ( ! class_exists( 'constructobuild_Theme_Info' ) ) {
    class constructobuild_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'constructo-build' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'constructo-build' );
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'constructo-build' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'constructo-build' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'constructo-build' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'constructo-build' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'constructo-build').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'constructo-build' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'constructo-build' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','constructo-build' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','constructo-build' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','constructo-build' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'constructo-build' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'constructo-build' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'constructo-build' ),
            'url' => esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/' )
        ),
        'pro_url'  => array(
            'text' => __( 'Buy Pro', 'constructo-build' ),
            'url' => esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/' )
        ),
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'constructo-build' ),
        'free_pro'             => esc_html__( 'Compare Free Vs Pro', 'constructo-build' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'constructo-build' ),
    ...
    'gs_steps' => array(
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'constructo-build' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'constructo-build' ),
            'button_label' => esc_html__( 'Browse Demos', 'constructo-build' ),
        ...
        'seven' => array(
            'title' => esc_html__( 'Need WordPress Theme Developemnt or Customization?', 'constructo-build' ),
            'desc' => esc_html__( 'We Create Beautiful & SEO Friendly WordPress Websites. Theme Customization to Custom WordPress Plugin Development.', 'constructo-build' ),
            'button_label' => esc_html__( 'Contact Now', 'constructo-build' ),
        ...
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'constructo-build' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'constructo-build' ),
            'button_label' => esc_html__( 'Rate Us', 'constructo-build' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'constructo-build' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'constructo-build' ),
            'button_label' => esc_html__( 'Get Support', 'constructo-build' ),
    ...
    'free_pro' => array(
        array(
            'desc'=> __( 'Access to All Starter Sites', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ), 
        array(
            'desc'=> __( 'Access to All Templates & Patterns', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Work with the Page Builders', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Creative & Unique Blocks', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Elementor Ready', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Priority Updates', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Priority Support', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Full Site Editing', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Customize Everything', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Lighting Fast', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'One-click Demo Installer', 'constructo-build' ),
            'free' => __( 'no','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Fully Responsive', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Clean & Optimized Code', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Multiple Site Styles', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Global Settings', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'CSS Animation', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Gutenberg Ready', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'eCommerce Ready', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'SEO Optimized', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'Multilingual Compatibility', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
        array(
            'desc'=> __( 'RTL/LTR Language Support', 'constructo-build' ),
            'free' => __( 'yes','constructo-build' ),
            'pro'  => __( 'yes','constructo-build' ),
        ),
    ),
