...
function bk_widgets_backend_enqueue()
{
    ...
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'constructo-build' ),
...
class constructobuild_Notice_Handler {
	...
	public function bk_getting_started() {
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'constructo-build' );
        ...
        if( $request == 2 && strpos( $slug, 'constructo-build' ) === false ){
            wp_send_json_error( );
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'constructo-build' );
