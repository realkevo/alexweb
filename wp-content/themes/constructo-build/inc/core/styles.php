 ...
 * @return void
 */
function constructobuild_get_color_value( $value ) {
...
function constructobuild_global_styles_vars() {
	if ( defined( 'constructobuild_VERSION' ) || ! function_exists( 'wp_get_global_styles' ) ) {
		return false;
	}
	...
	if ( ! empty( $core_button_styles['color']['background'] ) ) {
		$element_css_styles .= '--wp--custom--core-button--color--background: ' . constructobuild_get_color_value( esc_attr( $core_button_styles['color']['background'] ) ) . ';';
	}

	if ( ! empty( $core_button_styles['color']['text'] ) ) {
		// var(--wp--preset--color--vivid-red)
		$element_css_styles .= '--wp--custom--core-button--color: ' . constructobuild_get_color_value( esc_attr( $core_button_styles['color']['text'] ) ) . ';';
	}
	...
	if ( ! empty( $core_button_styles[':hover']['color']['background'] ) ) {
		$element_css_styles .= '--wp--custom--core-button--hover--color--background: ' . constructobuild_get_color_value( esc_attr( $core_button_styles[':hover']['color']['background'] ) ) . ';';
	}

	if ( ! empty( $core_button_styles[':hover']['color']['text'] ) ) {
		$element_css_styles .= '--wp--custom--core-button--hover--color: ' . constructobuild_get_color_value( esc_attr( $core_button_styles[':hover']['color']['text'] ) ) . ';';
	}
...
add_action( 'wp_enqueue_scripts', 'constructobuild_global_styles_vars', 100 );
